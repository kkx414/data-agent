...
async def merge_retrieved_info(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    图中位置：
        上游 recall_column / recall_metric / recall_value（三路召回结果的
        fan-in 汇合点）；下游 filter_table / filter_metric（并行执行，
        分别在表与指标维度上做裁剪）。
    ...
    try:
        ...
        logger.info(f"合并召回信息成功：{len(table_infos)} 张表, {len(metric_infos)} 个指标")
